...
export class MorpionComponent implements OnInit, AfterViewInit {
  ...
  //trigger quand le coup est valable et que c'est pas une win ou une draw
  randomSound = () => {
    if (Math.floor(Math.random() * 5) === 1) {
      this.sounds.getRandomFelicitation().play();
    }
  };
  ...
  endTurn = () => { //fonction principale de fin de tour, on fait plein de trucs et call plein de fonctions
